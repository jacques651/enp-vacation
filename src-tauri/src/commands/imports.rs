...
pub async fn import_cycles(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let nb_classe = row.get("nb_classe").and_then(|v| v.as_u64()).or_else(|| {
            row.get("nb_classe")
                .and_then(|v| v.as_str())
        ...
        let Some(nb_classe) = nb_classe else {
            errors.push(format!(
                "Ligne {}: nb_classe manquant ou invalide",
                line_num
            ));
            continue;
        };
        ...
        let exists: i64 =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM cycles WHERE designation = ?)")
                .bind(&designation)
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: cycle '{}' existe déjà",
                line_num, designation
            ));
            continue;
        }

        let result = sqlx::query("INSERT INTO cycles (designation, nb_classe) VALUES (?, ?)")
            .bind(&designation)
...
pub async fn import_modules(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let cycle_id = row.get("cycle_id").and_then(|v| v.as_u64());
        ...
        let actual_cycle_id = if let Some(id) = cycle_id {
            let exists: i64 =
                sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM cycles WHERE id = ?)")
                    .bind(id as i32)
                    .fetch_one(&state.pool)
                    .await
                    .map_err(|e| e.to_string())?;
            ...
            let result =
                sqlx::query_scalar::<_, i32>("SELECT id FROM cycles WHERE designation = ?")
                    .bind(&name)
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: module '{}' existe déjà pour ce cycle",
                line_num, designation
            ));
            continue;
        }

        let result = sqlx::query("INSERT INTO modules (designation, cycle_id) VALUES (?, ?)")
            .bind(&designation)
            .bind(actual_cycle_id)
            .execute(&state.pool)
            .await;
...
pub async fn import_matieres(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let vhoraire = row.get("vhoraire").and_then(|v| v.as_f64()).or_else(|| {
            row.get("vhoraire")
                .and_then(|v| v.as_str())
                .and_then(|s| s.parse().ok())
        });

        let module_id = row.get("module_id").and_then(|v| v.as_u64());
        ...
        let actual_module_id = if let Some(id) = module_id {
            let exists: i64 =
                sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM modules WHERE id = ?)")
                    .bind(id as i32)
                    .fetch_one(&state.pool)
                    .await
                    .map_err(|e| e.to_string())?;
            ...
            let result =
                sqlx::query_scalar::<_, i32>("SELECT id FROM modules WHERE designation = ?")
                    .bind(&name)
                    .fetch_optional(&state.pool)
                    .await
                    .map_err(|e| e.to_string())?;
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: matière '{}' existe déjà pour ce module",
                line_num, designation
            ));
            continue;
        }
        ...
        let result =
            sqlx::query("INSERT INTO matieres (designation, vhoraire, module_id) VALUES (?, ?, ?)")
                .bind(&designation)
                .bind(vhoraire)
                .bind(actual_module_id)
                .execute(&state.pool)
...
// ================= IMPORT PLAFONDS (VERSION SIMPLIFIÉE) =================
#[tauri::command]
pub async fn import_plafonds(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let volume_horaire_max = row.get("volume_horaire_max").and_then(|v| v.as_u64());

        let Some(titre_raw) = titre_raw else {
            errors.push(format!("Ligne {}: titre manquant", line_num));
            continue;
        };

        let Some(statut_raw) = statut_raw else {
        ...
        let Some(volume_horaire_max) = volume_horaire_max else {
            errors.push(format!(
                "Ligne {}: volume_horaire_max manquant ou invalide",
                line_num
            ));
            continue;
        };
        ...
        let titre = match titre_raw.to_lowercase().as_str() {
            t if t.contains("agent") => "agent",
            t if t.contains("directeur") => "directeur",
            t if t.contains("chef de service") => "chef de service",
            t if t.contains("chef de division") => "chef de division/service",
            t if t.contains("retraité") || t.contains("retraite") => "retraité",
            _ => "autre",
        }
        .to_string();

        // Normalisation du statut (simplifiée)
        let statut = match statut_raw.to_lowercase().as_str() {
            s if s.contains("interne") => "interne",
            s if s.contains("externe") => "externe",
            _ => {
                errors.push(format!(
                    "Ligne {}: statut '{}' invalide (doit être: interne ou externe)",
                    line_num, statut_raw
                ));
                continue;
            }
        ...
        if volume_horaire_max == 0 {
            errors.push(format!(
                "Ligne {}: volume_horaire_max doit être > 0",
                line_num
            ));
            continue;
        }
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: plafond '{}' pour statut '{}' existe déjà",
                line_num, titre, statut
            ));
            continue;
        }
...
// ================= IMPORT ENSEIGNANTS (VERSION SIMPLIFIÉE) =================
#[tauri::command]
pub async fn import_enseignants(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        // Récupérer le vh_max
        let vh_max = sqlx::query_scalar::<_, i64>(
        ...
        let Some(vh_max) = vh_max else {
            errors.push(format!(
                "Ligne {}: plafond non trouvé pour titre='{}' statut='{}'",
                line_num, titre, statut
            ));
            continue;
        };
        ...
        let result = sqlx::query(
            "INSERT INTO enseignants (nom, prenom, telephone, titre, statut, vh_max) VALUES (?, ?, ?, ?, ?, ?)"
        )
        .bind(&nom)
        .bind(&prenom)
        .bind(telephone)
...
pub async fn import_banques(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let exists: i64 =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM banques WHERE designation = ?)")
                .bind(&designation)
                .fetch_one(&state.pool)
                .await
                .map_err(|e| e.to_string())?;
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: banque '{}' existe déjà",
                line_num, designation
            ));
            continue;
        }

        let result = sqlx::query("INSERT INTO banques (designation) VALUES (?)")
            .bind(&designation)
            .execute(&state.pool)
            .await;
...
pub async fn import_promotions(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let exists: i64 =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM promotions WHERE libelle = ?)")
                .bind(&libelle)
                .fetch_one(&state.pool)
                .await
                .map_err(|e| e.to_string())?;
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: promotion '{}' existe déjà",
                line_num, libelle
            ));
            continue;
        }

        let result = sqlx::query("INSERT INTO promotions (libelle) VALUES (?)")
            .bind(&libelle)
            .execute(&state.pool)
            .await;
...
pub async fn import_annees_scolaires(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let exists: i64 =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM annees_scolaires WHERE libelle = ?)")
                .bind(&libelle)
                .fetch_one(&state.pool)
                .await
                .map_err(|e| e.to_string())?;
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: année scolaire '{}' existe déjà",
                line_num, libelle
            ));
            continue;
        }

        let result = sqlx::query("INSERT INTO annees_scolaires (libelle) VALUES (?)")
            .bind(&libelle)
            .execute(&state.pool)
            .await;
...
// ================= IMPORT COMPTES BANCAIRES (CORRIGÉ) =================
#[tauri::command]
pub async fn import_comptes_bancaires(
    ...
    for (idx, row) in data.iter().enumerate() {
        ...
        let enseignant_nom = row
            .get("enseignant_nom")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let enseignant_prenom = row
            .get("enseignant_prenom")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let banque_designation = row
            .get("banque_designation")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let numero_compte = row
            .get("numero_compte")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        // Correction: cle_rib doit être String
        let cle_rib = row
            .get("cle_rib")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        let enseignant_id =
            sqlx::query_scalar::<_, i32>("SELECT id FROM enseignants WHERE nom = ? AND prenom = ?")
                .bind(&enseignant_nom)
        ...
        let Some(enseignant_id) = enseignant_id else {
            errors.push(format!(
                ...
                line_num, enseignant_nom, enseignant_prenom
            ));
            continue;
        };
        ...
        let banque_id =
            sqlx::query_scalar::<_, i32>("SELECT id FROM banques WHERE designation = ?")
        ...
        if exists > 0 {
            errors.push(format!(
                "Ligne {}: compte bancaire pour '{} {}' existe déjà",
                line_num, enseignant_nom, enseignant_prenom
            ));
            continue;
        }

        // Insertion avec cle_rib en String
        let result = sqlx::query(
            "INSERT INTO comptes_bancaires (enseignant_id, banque_id, numero_compte, cle_rib, actif) VALUES (?, ?, ?, ?, 1)"
        )
        .bind(enseignant_id)
        .bind(banque_id)
        .bind(&numero_compte)
